   ...
   local tableView = widget.newTableView(
    {
        ...
        width = arr.w or 300,
        height =arr.h or 400,
        onRowRender = arr.row_render or nil,
        onRowTouch = arr.on_touch or nil,
        listener = arr.listener or nil
    }
  )---end newTableView
  data = arr.data or nil
  yrows = arr.data or 1
  -- Insert 40 rows
  for i = 1, #data do
      -- Insert a row into the tableView
      tableView:insertRow{params={data[i]}}
  end--for
...
function ye:set_rows(tbl,data)
  tbl:deleteAllRows()
  for i = 1, #data do
    -- Insert a row into the tableView
    tbl:insertRow{params={data[i]}}
  end--for
end--end set_rows
